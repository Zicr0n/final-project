...
io.on('connection', async (socket) => {
    ...
    socket.on('join_room', async ({ roomId, password }) => {
        ...
        if (currentRoomId && currentRoomId !== roomId) await removePlayerFromRoom(socket);

        const [foundRoom] = await db
            .select({ id: room.id, maxPlayers: room.maxPlayers, isPrivate: room.isPrivate, passwordHash: room.passwordHash, type: room.type, ownerId: room.ownerId })
            .from(room).where(eq(room.id, roomId));

        if (!foundRoom) { socket.emit('room_error', { error: 'Room not found' }); return; }

        if (!rooms[roomId]) {
            const mode = gameModes[foundRoom.type as keyof typeof gameModes];
            rooms[roomId] = { players: {}, timer: null, started: false, roomType: foundRoom.type, gameState: mode?.initMode() ?? null, owner: null, prompts: null, countdown: false };
        }

        if (foundRoom.isPrivate && foundRoom.passwordHash) {
            if (!password) { socket.emit('room_error', { error: 'Password required' }); return; }
            const ok = await argon2.verify(foundRoom.passwordHash, password);
            if (!ok) { socket.emit('room_error', { error: 'Wrong password' }); return; }
        }

        const currentRoom = rooms[roomId];
        let roomSockets = roomUserSockets.get(roomId);
        if (!roomSockets) { roomSockets = new Map(); roomUserSockets.set(roomId, roomSockets); }
        ...
        if (!alreadyInRoom && Object.keys(currentRoom.players).length >= foundRoom.maxPlayers) {
            socket.emit('room_error', { error: 'Room is full' }); return;
        }
        ...
        currentRoom.players[socket.data.userId] = { id: socket.data.userId, username: socket.data.username, joined: false, lives: 0, imageUrl: socket.data.imageUrl };
...
// SvelteKit handler must come after socket.io
app.use(handler);
