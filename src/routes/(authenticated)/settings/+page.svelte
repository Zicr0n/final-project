 ...
 <h1 class="text-4xl">{data.user.name}</h1>
    ...
    <form method="POST" onsubmit={ChangeUsername}>
        <input value={data.user.name} maxlength="16" minlength="1" name="username">
     ...
     <form method="POST" action="?/changePassword" class="input" onsubmit={handleSubmit}>
        ...
        <input minlength="8" bind:value={newPasswordConfirm} name="newPasswordConfirm" type="password">
        ...
        <ul>
            {#each sessions as session}
                ...
                <p>IP Address : {session.ipAddress}</p>
                    <form>
                        <button class="bg-cyan-500" onclick={()=> RemoveSession(session.token)}>Remove Session</button>
                    </form>
            {/each}
