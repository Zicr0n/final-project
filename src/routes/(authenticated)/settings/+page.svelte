...
<section class="space-y-6 rounded bg-surface-200-800 p-3">
			...
			{#if editUsername}
				<form method="POST" action="?/changeUsername" class="space-y-1">
					...
					<div class="flex justify-end gap-1">
						...
						>
						<button
							class="btn rounded preset-filled-error-400-600 btn-sm active:scale-95"
							onclick={OnCancelUsernameChange}>Cancel</button
						>
					</div>
				</form>
			{:else}
				...
				<button
					class="btn rounded preset-filled-primary-400-600 btn-sm active:scale-95"
					onclick={() => (editUsername = true)}>Edit</button
				>
			{/if}
		</div>
	</div>
		...
		<form method="POST" action="?/uploadProfile" enctype="multipart/form-data">
			...
			>
				...
				<div class="flex gap-1">
					...
					<button type="submit" class="btn-icon btn preset-filled-success-500"><Check /></button>
				</div>
			</FileUpload>
		</form>
	</div>
	<div class="flex items-center justify-between">
		...
		<Dialog>
			<Dialog.Trigger class="btn preset-filled-error-400-600 btn-sm uppercase"
				>sign out</Dialog.Trigger
			>
			<Portal>
