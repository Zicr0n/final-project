...
import { pgTable, text, timestamp, boolean, index, integer, varchar, pgEnum } from "drizzle-orm/pg-core";
...
export const galleryImageRelations = relations(galleryImage, ({ one }) => ({
  user: one(galleryImage, { fields: [galleryImage.userId], references: [user.id] }),
}));

export const friendRequestRelations = relations(friendRequest, ({ one }) => ({
  sender:   one(user, { fields: [friendRequest.senderId],   references: [user.id], relationName: "sender" }),
  receiver: one(user, { fields: [friendRequest.receiverId], references: [user.id], relationName: "receiver" }),
}));